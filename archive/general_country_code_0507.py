...
for index, row in iso_df.iterrows():
    ...
    # Iterate through each combination
        for pc_cols, cb_cols in combos:
            merged_df = exact_match(codes, merged_df, pc_cols, cb_cols, 12)
    ...
    merged_df.to_csv('0507/' + iso3 + '_processed.csv')
...
nan_counts_df_pc.to_csv('output/nan_counts_per_country_PC_0507.csv', index=False)
nan_counts_df_l.to_csv('output/nan_counts_per_country_L_0507.csv', index=False)