...
for index, row in iso_df.iterrows():
    ...
    # `admin_name2`
    codes1 = codes.copy()
    codes1 = codes1[['postal_code', 'latitude', 'longitude', 'admin_name1', 'admin_name2']]
    ...
    # `admin_name3`
    codes2 = codes.copy()
    codes2 = codes2[['postal_code','admin_name1', 'admin_name3', 'latitude', 'longitude']]

    codes2['admin_name1'] = codes2['admin_name1'].str.lower()
    codes2['admin_name3'] = codes2['admin_name3'].str.lower()

    codes2.drop_duplicates(subset=['admin_name1', 'admin_name3'], inplace=True)

    merged_df = pd.merge(merged_df, codes2, left_on=['city', 'region'], right_on=['admin_name3', 'admin_name1'], how='left')
    ...
    # `place_name`
    codes3 = codes.copy()
    codes3 = codes3[['postal_code','admin_name1', 'place_name', 'latitude', 'longitude']]

    codes3['admin_name1'] = codes3['admin_name1'].str.lower()
    codes3['place_name'] = codes3['place_name'].str.lower()

    codes3.drop_duplicates(subset=['admin_name1', 'place_name'], inplace=True)

    merged_df = pd.merge(merged_df, codes3, left_on=['city', 'region'], right_on=['place_name', 'admin_name1'], how='left')
    ...
    # `place_name` ONLY
    codes3.drop_duplicates(subset=['place_name'], inplace=True)
    merged_df = pd.merge(merged_df, codes3, left_on=['city'], right_on=['place_name'], how='left')
    ...
    merged_df = pd.merge(merged_df, codes4, left_on=['postal_code_y'], right_on=['postal_code'], how='left')
